<template>
    <main class="wrapper animate__animated animate__zoomIn">
        <DisplayComponent :is-equation-finished="isEquationFinished" :equation-actual-value="equationActualValue"
            :lastEquation="lastEquation" />
        <div class="button-container">
            <div class="button-row" v-for="(ButtonRow, index) in buttonMapping" :key="index">
                <ButtonComponent @button-click="addToDisplay" v-for="(buttonValue, index) in ButtonRow"
                    :key="index + buttonValue" :value="buttonValue" />
            </div>
        </div>
    </main>
</template>
...
export default {
    ...
    methods: {
        ...
        evaluateEquation() {
            ...
            this.equationActualValue = this.equationActualValue.replace(/×/g, '*').replace(/÷/g, '/');
        ...
        addToDisplay(value) {
            ...
            if (Validator.isEquationInverterPressed(value) &&
                ...
                if (['-', '+', '×', '÷'].includes(lastChar)) {
                    ...
                    this.equationActualValue = this.equationActualValue.slice(0, -valueAfterOperator.length) + invertedNumber;
                }
                return;
            }
